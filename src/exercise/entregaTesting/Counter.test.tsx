...
import { render, fireEvent, screen } from "@testing-library/react";  // Importamos las funciones necesarias
import Counter from "../03-ReactDom/Counter";  // El componente que vamos a probar

describe("Counter", () => {
  test("should render Counter component and buttons", () => {
    render(<Counter />);  // Renderizamos el componente
  ...
  test("should increment the counter when the increment button is clicked", () => {
    render(<Counter />);

    const incrementButton = screen.getByRole("button", { name: /increment/i });
    const counterDisplay = screen.getByText(/counter: 0/i);

    // Hacemos click en el botón de incremento
    fireEvent.click(incrementButton);

    // Verificamos que el contador haya aumentado
    expect(counterDisplay).toHaveTextContent("Counter: 1");
  });

  test("should decrement the counter when the decrement button is clicked", () => {
    render(<Counter />);

    const decrementButton = screen.getByRole("button", { name: /decrement/i });
    const counterDisplay = screen.getByText(/counter: 0/i);

    // Hacemos click en el botón de decremento
    fireEvent.click(decrementButton);

    // Verificamos que el contador haya disminuido
    expect(counterDisplay).toHaveTextContent("Counter: -1");
  });
});
